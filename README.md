...
[![Live Demo](https://img.shields.io/badge/🌐_Live_Deployment-kaksha--ai--portal.web.app-teal?style=for-the-badge&logo=firebase)](https://kaksha-ai-portal.web.app)
[![Next.js 16](https://img.shields.io/badge/Next.js-16.3.5-black?style=for-the-badge&logo=next.js)](https://nextjs.org/)
[![Gemini 3.8 Flash](https://img.shields.io/badge/AI_Engine-Gemini_3.8_Flash-4285F4?style=for-the-badge&logo=google)](https://ai.google.dev/)
...
- **1-Click Evaluator Personas**: Pre-configured demo logins for **Dr. Anushka Yerpude** (Faculty) and **Vedanshi Puwar** (Student) for instant evaluation.
...
```mermaid
graph TD
    subgraph Client ["Next.js 16 Client Portal (Firebase Hosting)"]
        Landing["Split Auth Landing Page (IAR ERP)"]
        StaffPortal["Staff ERP Studio"]
        StudentPortal["Student Study Desk"]
    end

    subgraph Service ["Client-Side AI Service Layer (geminiService.ts)"]
        ResearchFn["runResearch()"]
        GenerateFn["runGeneratePack()"]
        TutorFn["runAskTutor()"]
        RegenFn["runRegenerate()"]
    end

    subgraph Gemini ["Google Gemini 3.8 Flash SDK (@google/genai)"]
        Grounding["Google Search Grounding Tool"]
        Structured["JSON Schema & Structured Output"]
        Roles["Multi-Mode Student Tutor Personas"]
    end

    subgraph Workspace ["Export Engine (googleWorkspace.ts)"]
        GDocs["Google Docs Builder"]
        GSlides["Google Slides Builder"]
        GForms["Google Forms Builder"]
        HTMLPrint["Print-Ready CSS Sheets"]
    end

    Landing -->|Dr. Yerpude Login| StaffPortal
    Landing -->|Vedanshi Puwar Login| StudentPortal

    StaffPortal --> ResearchFn
    StaffPortal --> GenerateFn
    StaffPortal --> RegenFn
    StaffPortal --> Workspace

    StudentPortal --> TutorFn

    ResearchFn --> Grounding
    GenerateFn --> Structured
    TutorFn --> Roles
```
...
| **Vedanshi Puwar** | Student (Sem 5) | `2024CS0182` | `student2024` | 30s Plain English, Interactive Formula Playground, 5-Q Quiz, AI Buddy |
